import { ProcessedReceipt, ReceiptItem, ReceiptParser } from '../types';

export class TradersReceiptParser implements ReceiptParser {
    canParse(text: string): boolean {
        return /트레이더스|TRADERS/i.test(text);
    }
    ...
    parse(text: string): ProcessedReceipt {
        ...
        // 9. 부가세 추출
        const vatPattern = /부\s*가\s*세\s*(\d{1,3}(?:,\s*\d{3})*)/;
        const vatMatch = cleanedText.match(vatPattern);
        if (!vatMatch) {
            throw new Error('부가세를 찾을 수 없습니다.');
        }
        const vatAmount = parseInt(vatMatch[1].replace(/[,\s]/g, ''));
    ...
    private extractItems(text: string): ReceiptItem[] {
        // 상품 목록 섹션 추출
        const itemSectionMatch = text.match(/상\s*품\s*명\s*단\s*가\s*수\s*량\s*_?\s*금\s*액([\s\S]*?)(?:종\s*품\s*목|합\s*계)/);
        if (!itemSectionMatch) {
            return [];
        }
        ...
        lines.forEach((line, index) => {
            ...
            const patterns = [
                ...
                /^[*\s]*(?:[0-9Q]+\s+)?([^._]+?)[\s_]*([0-9,]+)[\s_.]*([0-9]+)[\s_.]*([0-9,]+)/,
                // 패턴 3: 특수문자로 구분된 형식
                /^[*\s]*(?:[0-9Q]+\s+)?([^._]+)[._]([0-9,]+)[._]([0-9]+)[._]([0-9,]+)/,
                // 패턴 4: 상품명 숫자 가격 수량 금액
                /^[*\s]*(?:[0-9Q]+\s+)?([^0-9]+\s*\d+[^0-9]*)[\s_.]*([0-9,]+)[\s_.]*([0-9]+)[\s_.]*([0-9,]+)/,
                // 패턴 5: 특수문자(_)로 구분된 형식
                /^[*\s]*(?:[0-9Q]+\s+)?([^_]+)_([0-9,]+)_([0-9]+)_([0-9,]+)/,
                // 패턴 6: 상품명에 괄호와 숫자가 포함된 형식
                /^[*\s]*(?:[0-9Q]+\s+)?([^0-9]+(?:\s*\([^)]*\d+[^)]*\))?)[\s_.]*([0-9,]+)[\s_.]*([0-9]+)[\s_.]*([0-9,]+)/,
                // 패턴 7: 상품명 끝에 숫자가 있는 형식
                /^[*\s]*(?:[0-9Q]+\s+)?([^._]+?(?:\s*\d+(?:\.\d+)?[kKgGlLmMＬ]?)?)[\s_.]*([0-9,]+)[\s_.]*([0-9]+)[\s_.]*([0-9,]+)/,
                // 패턴 8: 상품명 앞에 특수문자가 있는 형식
                /^[*\s]*(?:[0-9Q]+\s+)?[*#@\[\])]?\s*([^0-9]+(?:\s*\d+(?:\.\d+)?[kKgGlLmMＬ]?)?)[\s_.]*([0-9,]+)[\s_.]*([0-9]+)[\s_.]*([0-9,]+)/,
                // 패턴 9: 상품명 앞에 숫자와 특수문자가 있는 형식
                /^[*\s]*(?:[0-9Q]+\s+)?[*]?\d+[_]?\s*([^0-9]+(?:\s*\d+(?:\.\d+)?[kKgGlLmMＬ]?)?)[\s_.]*([0-9,]+)[\s_.]*([0-9]+)[\s_.]*([0-9,]+)/
            ];
